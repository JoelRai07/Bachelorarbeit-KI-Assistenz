...
Die Projektstruktur folgt einer flachen Modulorganisation. Jedes Modul in \texttt{src/modules/} übernimmt sowohl die Datenerhebung als auch die Normalisierung in das \ac{UFS}:
...
Ergänzend enthält der System-Prompt sechs Priorisierungsregeln, mit denen das Must-have/Nice-to-have-Schema operationalisiert wird:
...
Ein vollständiger Durchlauf umfasst folgende Schritte:
\begin{enumerate}
...
Für den \ac{BWEC}, eine produktiv eingesetzte React-Anwendung des Landes Baden-Württemberg, erzeugt die Pipeline zehn axe-core-Violations, zwei Playwright-Checks und acht grep-Treffer. Da die Codebasis weit größer als die Codebasis der synthetischen Suites ist, wird sie in Chunks aufgeteilt; bei \texttt{maxfiles}~=~100 (66 Chunks) erzeugt der \acs{LLM}-Detektor im Mittel 15 bis 32 Findings pro Durchlauf. Bei \texttt{maxfiles}~=~500 (330 Chunks) steigt der Gesamtumfang entsprechend. Rohdaten und Modellübersichten beider Konfigurationen befinden sich in Anhang~\ref{app:rohdaten_bwec} und Anhang~\ref{app:modell_summary_bwec} (\texttt{maxfiles}~=~100) sowie in Anhang~\ref{app:rohdaten_bwec500} und Anhang~\ref{app:modell_summary_bwec500} (\texttt{maxfiles}~=~500).
