...
Das \acs{LLM}-Detektor-Modul ergänzt die regelbasierten Quellen um eine modellbasierte Quellcode\-analyse. Es wird nur ausgeführt, wenn der \ac{CLI}-Parameter \texttt{-{}-llm-detect} gesetzt ist und ein Quellcodepfad via \texttt{-{}-src-dir} vorliegt. Ziel ist nicht die Ersetzung bestehender Tools, sondern die Ergänzung um eine zusätzliche Detektionsquelle mit semantischem Fokus.
...
\begin{itemize}
  ...
  \item \textbf{\acs{LLM}-Detektor} arbeitet kontextsensitiv. Er kann auch semantisch ähnliche \mbox{Problemstellen} erkennen, die nicht exakt einem festen Regex-Muster entsprechen, ist jedoch anfälliger für Formatabweichungen und Halluzinationen.
\end{itemize}
