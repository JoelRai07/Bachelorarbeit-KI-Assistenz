...
Die Evaluation stützt sich ausschließlich auf die synthetisch konstruierte test-12-Suite; die Übertragbarkeit auf produktive Anwendungen mit größeren Codebasen und unbekannter Ground Truth ist daher nicht direkt ableitbar (vgl. Abschnitt~\ref{sec:limit_extern}). Ob die erzielten Recall-Werte auf einer produktiven Anwendung -- insbesondere dem \ac{BWEC} -- replizierbar sind, bleibt eine offene empirische Frage, die in den Folgestudien (Abschnitt~\ref{sec:eval_erweiterung}) adressiert wird. Zudem wurden ausschließlich \ac{CPU}-basierte Laufzeitmessungen erhoben; auf \ac{GPU}-beschleunigter Hardware wäre \ac{NFA}-02 für alle drei Modelle klar erfüllbar.
